...
import { markNotificationEventFailed } from "@/lib/notifications/repository";
...
export async function POST(
  ...
  try {
    ...
    await markNotificationEventFailed(id, body.error ?? "");

    return NextResponse.json({ ok: true }, { headers: corsHeaders });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "标记提醒事件发送失败状态失败" },
      { status: 500, headers: corsHeaders },
    );
  }
}
