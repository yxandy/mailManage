...
function detectDialCode(phoneNumber: string): string {
  return KNOWN_DIAL_CODES.find((dialCode) => phoneNumber.startsWith(dialCode)) ?? "";
}
...
export function splitPhoneNumberByKnownDialCode(phoneNumber: string): SplitPhoneNumberResult {
  const normalizedPhoneNumber = phoneNumber.replace(/\D/g, "");
  const detectedDialCode = detectDialCode(normalizedPhoneNumber);

  if (!detectedDialCode) {
    return {
      dialCode: "",
      localNumber: normalizedPhoneNumber || phoneNumber.trim(),
    };
  }

  return splitPhoneNumberByDialCode({
